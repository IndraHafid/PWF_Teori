  ...
  try {
    ...
    const data = await Promise.all(
      responses.map(async (res) => {
        ...
        return (await res.json()) as { results: Show[] }
      })
    )

    return {
      trending: data[0]?.results ?? [],
      topRated: data[1]?.results ?? [],
      netflix: data[2]?.results ?? [],
      action: data[3]?.results ?? [],
      comedy: data[4]?.results ?? [],
      horror: data[5]?.results ?? [],
      romance: data[6]?.results ?? [],
      docs: data[7]?.results ?? [],
    }
  } catch (error) {
...
export async function getNewAndPopularShows() {
  try {
    ...
    return {
      popularTvs: data[0]?.results ?? [],
      popularMovies: data[1]?.results ?? [],
      trendingTvs: data[2]?.results ?? [],
      trendingMovies: data[3]?.results ?? [],
    }
  } catch (error) {
    console.error("Error fetching new and popular shows:", error)
    return {
      popularTvs: [],
      popularMovies: [],
      trendingTvs: [],
